...
$errorMessage = null;
$InventoryInvoiceID = isset($_GET['InventoryInvoiceID']) ? intval($_GET['InventoryInvoiceID']) : null;
$Customer = null;

if ($InventoryInvoiceID) {
    ...
    $TotalPayment = $Prices->TotalPrice_for_each_product($InventoryInvoice[0]['InventoryInvoiceID']);
    $rest_of_the_payment = $TotalPayment - $CustomerPayment -$InventoryInvoice[0]['Discount'] ; 

} else {
    echo "Invalid InventoryInvoiceID.";
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['increase'])) {
  ...
  if (!empty($Payment)) {
      if($rest_of_the_payment>=$Payment){    
        if ($suppliersController->IncreasePaymentSupplier($Payment,$CustomerID,$InventoryInvoiceID)) {
        // Redirect to the same page to refresh and show new category
        header("Location: SuppliersPayDebt.php?InventoryInvoiceID=$InventoryInvoiceID");
        exit();
    }else
    {
      $errorMessage = "لا يمكن دفع مبلغ زيادة الي المورد";
    }
  }else{
    $errorMessage = "لا يمكن دفع مبلغ زيادة الي المورد";
  }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['decrease'])) {
  ...
  if (!empty($Payment)) {
      if ($suppliersController->DecreasePaymentSupplier($Payment,$CustomerID,$InventoryInvoiceID)) {
          // Redirect to the same page to refresh and show new category
                              ...
                              <div class="card-body">
                                <form id="formAccountSettings">
                                  <div class="row">
                                    ...
                                    <div class="mb-3 col-md-6">
                                      <label for="email" class="form-label">الديون للمورد</label>
                                      <input class="form-control" type="text" id="email" name="email" value="<?= htmlspecialchars($rest_of_the_payment)?>" readonly>
                                    </div>
                                    <div class="mb-3 col-md-6">
                                  ...
                                  <div class="card mb-4">
                                    ...
                                    <div class="card-body">
                                      <form method="post" action="">
                                          ...
                                          <div class="mt-3">
                                            <button type="submit" class="btn rounded-pill btn-success" name="increase">دفع</button>
                                            <button type="submit" class="btn rounded-pill btn-danger" name="decrease">خصم من المدفوعات</button>
                                          </div>
                                      </form>
                                    </div>
                                  </div>
