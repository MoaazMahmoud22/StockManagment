...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_customer'])) {
    ...
    if (!empty($CustomerName) && !empty($phone_number)) {
        if (!$CustomerController->isCustomerNameExist($CustomerName)) {
            if ($CustomerController->addCustomer($CustomerName, $phone_number)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_customer'])) {
    ...
    if (!empty($CustomerID) && !empty($CustomerName) && !empty($phone_number)) {
        if (!$CustomerController->isCustomerNameExist($CustomerName,$CustomerID)) {
            $CustomerController->updateCustomer($CustomerID, $CustomerName, $phone_number);
            header("Location: " . $_SERVER['PHP_SELF']);
            exit();
        } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_customer'])) {
    ...
    if (!empty($CustomerID)) {
        $CustomerController->DeActiveCustomer($CustomerID);
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    } else {
...
// Pagination logic
$totalCustomers = count($Customers); // Total number of customers
...
$currentCustomers = array_slice($Customers, $startIndex, $CustomerPerPage);
                                  ...
                                  <div class="card mb-4">
                                    ...
                                    <div class="card-body">
                                      <form method="post" action="">
                                        <div class="mb-3">
                                          <label class="form-label" for="Customer_name">اسم العميل</label>
                                          <input type="text" class="form-control" id="Customer_name" name="Customer_name" placeholder="اسم العميل" required>
                                        </div>
                                        <div class="mb-3">
                                          <label class="form-label" for="Phone_number">رقم الهاتف</label>
                                          <input type="text" class="form-control" id="Phone_number" name="Phone_number" placeholder="رقم الهاتف للعميل" required>
                                        </div>
                        ...
                        <div class="table-responsive text-nowrap">
                          <table class="table table-striped">
                            ...
                            <?php if ($currentCustomers):
                                        $index = 0; ?>
                                        <?php foreach ($currentCustomers as $Customer): 
                                          $index++;?>
                                                  <tr>
                                  ...
                                  <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                      <!-- Modal Trigger -->
                                      <button type="button" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#editCategoryModal-<?php echo htmlspecialchars($Customer['CustomerId']); ?>">
                                      ...
                                      <form method="post" action="" style="display: inline;">
                                                          ...
                                                          <button type="submit" class="dropdown-item" name="delete_customer" onclick="return confirm('هل انت متاكد من حذف هذا العميل من هذه القائمة؟');">
                                                            <i class="bx bx-trash me-1"></i> الغاء التفعيل
                                                          </button>
                                      </form>
                                    </div>
                                    <div class="modal fade" id="editCategoryModal-<?php echo htmlspecialchars($Customer['CustomerId']); ?>" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
                                        ...
                                        <form class="modal-content" method="post" action="">
                                            ...
                                            <div class="modal-body">
                                              <div class="row">
                                                <div class="col mb-3">
                                                      <label for="nameBackdrop-<?php echo htmlspecialchars($Customer['CustomerId']); ?>" class="form-label">اسم العميل</label>
                                                      <input type="text" id="nameBackdrop-<?php echo htmlspecialchars($Customer['CustomerId']); ?>" class="form-control" name="CustomerName" value="<?php echo htmlspecialchars($Customer['CustomerName']); ?>" required>
                                                  </div>
                                                </div>
                                              <div class="row">
                                                <div class="col mb-3">
                                                      <label for="nameBackdrop-<?php echo htmlspecialchars($Customer['phone_number']); ?>" class="form-label">رقم الهاتف</label>
                                                      <input type="text" id="nameBackdrop-<?php echo htmlspecialchars($Customer['phone_number']); ?>" class="form-control" name="phone_number" value="<?php echo htmlspecialchars($Customer['phone_number']); ?>" required>
                                                  </div>
                                                </div>
                                              </div>
                                                <div class="modal-footer">
                                                            ...
                                                            <button type="submit" class="btn btn-primary" name="update_customer">Save</button>
                                                  </div>
                                                    <input type="hidden" name="CustomerId" value="<?php echo htmlspecialchars($Customer['CustomerId']); ?>">
